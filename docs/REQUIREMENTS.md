...
1. Captain agent — role implementation with two operational modes (planning mode for expedition decomposition into missions, operations mode for routine triage), system prompt with full state visibility. Owns `expeditions/` and `station-logs/`, can read everything. Wired to frontier model via `captain` config role. Does NOT write wiki articles, process raw sources, manage the search index, or edit files owned by other agents. Accepts human directives (e.g., "start the expedition", "prioritize X") and translates them into missions.
2. Captain toolkit — five deterministic tools: (a) mission queue manager (priority queue, dependency graph, topological sort), (b) mission ledger (SQLite source of truth for mission state, dual-write with YAML audit trail — ledger is authoritative, YAML is human-readable record), (c) token budget tracker (running tally per agent and per expedition, alerts at configurable thresholds), (d) schedule runner (cron expression evaluation — `matches()` and `next_run()` for 5-field cron; trigger matching and actual scheduled execution deferred to Phase 7), (e) status aggregator (reads mission ledger + agent statuses, produces LLM-digestible structured summaries for Captain reasoning, rolls up agent checklists into mission-level acceptance criteria views)
3. Iterative planning — named iteration phases during build: **Discovery** (Scout ingests all sources), **Structuring** (Captain identifies which raw sources need compilation — scope decision; Compiler plan mode proposes how to compile them — editorial decision on article types, groupings, titles; Captain reviews, sequences, and creates missions from the Compiler's plan with dependency ordering), **Refinement** (Archivist indexes compiled articles, Curator retroactive cross-referencing over full corpus — intentionally batched here, not incremental during Discovery/Structuring, because multi-pass retrieval against an incomplete corpus is wasteful; Inspector sweep as hook/placeholder for Phase 6). The Structuring split enforces the spec's role boundary: Captain orchestrates scope and sequencing, Compiler makes editorial decisions. The expedition's `purpose` field (see #4) flows through to both — Captain uses it to decide scope, Compiler uses it as editorial lens during plan mode and compilation. Captain's observe step is an LLM call reading summaries and making replanning decisions, not just status checking. Iteration count is variable — small expeditions may complete in 2, large ones may need 4-5. Exit condition: all sources processed, all linking passes done, Inspector sweep clean or findings addressed.
4. Expedition lifecycle — full `expedition.yaml` parsing including purpose (a statement of why this knowledge base exists and who it serves — e.g., "Build a reference guide for active treasure hunters preparing for a physical search"), scope (description + keywords — the domain), sources (local with path/pattern/watch; RSS, GitHub, web sources parsed but deferred to Phase 7), stationed config (schedule, triggers, resources with `daily_token_budget` and `max_concurrent_missions`, reporting). The `purpose` field is required and distinct from scope: scope describes what the knowledge base covers, purpose describes why it exists. Purpose flows through to Captain planning prompts (informing scope decisions and acceptance criteria), Compiler system prompt (as editorial lens for article content and tone), and verification (criteria checked against the expedition's stated purpose, not the Captain's imagination). Build phase orchestration across all agents. `expedition create` sets up directory structure under `expeditions/<name>/` (plan.yaml, missions/, review/). Phase transition (build → stationed) as explicit human decision with build report summarizing: scope and sources, article counts and word totals, token usage by agent, per-iteration breakdown of completed/failed missions, and overall coverage metrics. Extend `ExpeditionConfig` model to include purpose, stationed.resources and scaling config.
5. Mission state machine — Mission model with full field set: id, agent, type, status (pending/running/completed/failed/stale), priority, inputs, acceptance_criteria, checklist (agent-populated at mission start), created_by, created_at. Two-level completion: agent self-declares against checklist, Captain verifies against mission-level criteria. Status transitions: pending→running→completed, running→failed (transient: retry max 3, deterministic: review queue), completed→stale (when inputs change). Failure record in YAML audit: type, error, retries, failed_at. Missions contain ordered task sequences; tasks use Phase 2 TaskRunner. Task-level failures roll up to mission status.
6. Task dependency resolution — topological sort on a dependency graph. Captain's observation LLM call during Structuring iteration produces dependency edges (concept A references concept B). Foundational concepts compiled before articles that reference them. Infrastructure for cascading task chains from Curator proposals (Curator-proposal trigger wired in Phase 6, but the multi-mission dependency chain structure — compile → reindex → link — built now).
7. Deterministic scaling system — config-driven, no LLM calls in scaling path. Parse `scaling` config block: per-agent scalability (scout/compiler/explorer: auto), auto_scale (queue_depth trigger, max_instances, cooldown_minutes). Singleton enforcement for Captain, Curator, AND Inspector (encode Inspector singleton now for Phase 6 inheritance). Archivist is a system component, not subject to scaling. Enable WAL mode on all SQLite databases used by concurrent agents. Note: SQLite write concurrency is a known ceiling (see spec) — LLM API latency >> DB write time makes this acceptable for v1 CLI.
8. Deterministic budget system — reads config from `scaling.budget`. Two distinct behaviors: warning at configurable threshold (default 0.8 = 80% spend, generates Captain notification) and hard cap at daily_token_limit (halts agent execution). Per-agent AND per-expedition tracking with date dimension. Daily reset.
9. CLI: `assistonauts expedition create --config <path>` — copies/validates expedition config into `expeditions/<name>/`, creates directory structure. `assistonauts build <expedition-name>` — runs build phase with iterative planning. `--dry-run` flag shows the Discovery iteration plan without executing any missions.
10. LLM call tracing — observer callback on `LLMClient.complete()` that fires on every LLM call with full input (messages, system prompt, model) and output (response content, token usage). Opt-in via `on_llm_call` callback parameter. The orchestrator wires this to a JSONL trace file at `expeditions/<name>/llm-trace.jsonl` during builds. Captures all non-deterministic operations across all agents and layers (planning, compilation, verification, RAG triage/scoring, image conversion) without requiring agent code changes. Each trace entry includes timestamp, caller context (agent role, mission ID when available), and the complete prompt/response pair. The trace file also includes mission-level lifecycle events (`mission_start`, `mission_complete`, `mission_failed`, `mission_skipped`) so that missions which complete without LLM calls (e.g. curator finding no new links) are still visible in the trace. The trace file is the single source of truth for debugging both LLM behavior and mission lifecycle end-to-end.
...
**Validation:** Create an expedition config, Captain produces an iterative plan (Discovery → Structuring → Refinement), executes end-to-end with proper sequencing. Verify named iterations produce different agent compositions. Test scaling with concurrent Compiler instances on a large source batch. Verify budget tracking halts at limit.
