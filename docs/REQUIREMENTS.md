...
3. Iterative planning — named iteration phases during build: **Discovery** (Scout ingests all sources, Compiler compiles first batch, Archivist indexes), **Structuring** (Captain LLM reads compiled summaries, identifies foundational concepts, sequences remaining compilations with dependency ordering, identifies structural needs), **Refinement** (Curator retroactive cross-referencing over full corpus — intentionally batched here, not incremental during Discovery/Structuring, because multi-pass retrieval against an incomplete corpus is wasteful; Inspector sweep as hook/placeholder for Phase 6). Captain's observe step is an LLM call reading summaries and making replanning decisions, not just status checking. Iteration count is variable — small expeditions may complete in 2, large ones may need 4-5. Exit condition: all sources processed, all linking passes done, Inspector sweep clean or findings addressed.
4. Expedition lifecycle — full `expedition.yaml` parsing including scope (description + keywords), sources (local with path/pattern/watch; RSS, GitHub, web sources parsed but deferred to Phase 7), stationed config (schedule, triggers, resources with `daily_token_budget` and `max_concurrent_missions`, reporting). Build phase orchestration across all agents. `expedition create` sets up directory structure under `expeditions/<name>/` (plan.yaml, missions/, review/). Phase transition (build → stationed) as explicit human decision with build report summarizing: scope and sources, article counts and word totals, token usage by agent, per-iteration breakdown of completed/failed missions, and overall coverage metrics. Extend `ExpeditionConfig` model to include stationed.resources and scaling config.
